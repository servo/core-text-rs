...
pub type CTFontAttributes = CFDictionary<CFStringRef, CFTypeRef>;
...
impl CTFontTraits : TraitAccessors {
    priv fn extract_number_for_key(key: CFStringRef) -> CFNumber {
        ...
        assert wrapper.type_id() == CFNumber::type_id();
        CFWrapper::from_CFType::<CFNumberRef, (), ()>(wrapper)
    }
...
pub impl CTFontDescriptor : CTFontDescriptorMethods {
    static fn new_from_attributes(attributes: &CTFontAttributes) -> CTFontDescriptor {
        let result = CTFontDescriptorCreateWithAttributes(*attributes.borrow_ref());
    ...
    priv fn get_string_attribute(attribute: CFStringRef) -> Option<~str> {
        ...
        let cfstr: CFString = CFWrapper::from_CFType::<CFString, (), ()>(move cfty);
    ...
    fn font_path() -> ~str {
        ...
        let cfurl : CFURL = CFWrapper::from_CFType::<CFURL, (), ()>(move cfty);
        return cfurl.to_str();
    }
}
